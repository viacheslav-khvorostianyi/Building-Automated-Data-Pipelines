...
def weather_processing_dag():
    ...
    def transform(logical_date: str) -> str:
        ...
        if hook.get_first(
            ...
            logging.info("transformed_weather already present for %s %s - skipping.", CITY, logical_date)
            return logical_date
        ...
        if not row:
            raise ValueError(f"No raw_weather row found for {CITY} {logical_date}")
        ...
        hook.run(
            ...
            """,
            parameters=(CITY, logical_date, json.dumps(clean)),
        )
        logging.info("Stored transformed_weather for %s %s.", CITY, logical_date)
        return logical_date

    # ── 3. DATA QUALITY CHECK ─────────────────────────────────────────────────
    @task(execution_timeout=timedelta(minutes=10))
    def quality_check(logical_date: str, min_temp: float = -80.0, max_temp: float = 60.0) -> str:
        ...
        if not row:
            raise ValueError(f"No transformed row for {CITY} {logical_date}")
        ...
        if errors:
            raise ValueError(f"Quality FAILED for {CITY} {logical_date}: " + "; ".join(errors))
        ...
        logging.info("Quality check PASSED for %s %s.", CITY, logical_date)
        return logical_date
    ...
    def normal_load(logical_date: str) -> None:
        _upsert_final(logical_date, alert=False)
    ...
    def alert_load(logical_date: str) -> None:
        ...
        logging.warning(
            ...
            CITY, data["wind_speed"], logical_date,
        )
        _upsert_final(logical_date, alert=True)

    def _upsert_final(logical_date: str, *, alert: bool) -> None:
        hook = pg()
        ...
        logging.info("Loaded weather for %s %s (alert=%s).", CITY, logical_date, alert)

    # ── Wire up ────────────────────────────────────────────────────────────────
    ensure_tables_task = ensure_tables()
    raw_ld             = transform(logical_date="{{ ds }}")
